...
<div class="profile-page-full w-full min-h-full px-4 py-6 md:px-6 md:py-8">
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {{-- Left: Profile info (institution, school, department — schools/depts are the app backbone) --}}
        <div class="rounded-lg border border-gray-200 bg-white p-4 min-w-0">
            ...
            <dl class="grid grid-cols-1 gap-1.5 sm:grid-cols-2 text-xs">
                ...
                <div><dt class="text-gray-500">School</dt><dd class="font-medium text-gray-900 uppercase">{{ $user->department?->school?->name ? Str::upper($user->department->school->name) : '—' }}</dd></div>
            ...
            <div class="mt-3 flex flex-col gap-2 border-t border-gray-100 pt-3">
                ...
                <div class="p-2 rounded-md bg-orange-50 border border-orange-200">
                    <p class="text-xs text-orange-800">Select your <strong>school</strong> and <strong>department</strong> to complete your profile.</p>
                </div>
                @endif
                <a href="{{ route('dashboard.users.edit', $user) }}" class="text-xs text-primary-600 hover:text-primary-800 font-medium underline w-fit">
