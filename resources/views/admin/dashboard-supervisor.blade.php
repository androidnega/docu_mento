...
@endphp
    ...
    <section class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-2">
            <div>
                <h2 class="text-sm font-semibold text-slate-800 dark:text-slate-100">Supervisors</h2>
                <p class="text-xs text-slate-500 dark:text-slate-300 mt-0.5">All supervisors and their allocations.</p>
            </div>
            <a href="{{ route('dashboard.docu-mentor.supervisors.index') }}" class="text-sm font-medium text-primary-600 hover:text-primary-700">Open full directory</a>
        </div>
        @if(($supervisorsDirectory ?? collect())->isEmpty())
            <div class="p-6 text-sm text-slate-500 dark:text-slate-300">No supervisors found.</div>
        @else
            <div class="p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                @foreach($supervisorsDirectory as $sup)
                    @php
                        $isCurrentSupervisor = $user && (int) $user->id === (int) $sup->id;
                    @endphp
                    <article class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0">
                                <p class="truncate text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $sup->name ?? $sup->username ?? '—' }}</p>
                                <p class="truncate text-xs text-slate-500 dark:text-slate-300 mt-0.5">{{ $sup->username ?? '—' }}</p>
                            </div>
                            @if($isCurrentSupervisor)
                                <span class="inline-flex items-center rounded-full border border-amber-300 bg-amber-50 px-2 py-0.5 text-[11px] font-medium text-amber-800">You</span>
                            @endif
                        </div>
                        <div class="mt-3 grid grid-cols-2 gap-2">
                            <div class="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-2.5 py-2">
                                <p class="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">Students</p>
                                <p class="mt-1 text-base font-semibold tabular-nums text-slate-900 dark:text-slate-100">{{ (int) ($sup->allocated_students_count ?? 0) }}</p>
                            </div>
                            <div class="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-2.5 py-2">
                                <p class="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">Projects</p>
                                <p class="mt-1 text-base font-semibold tabular-nums text-slate-900 dark:text-slate-100">{{ (int) ($sup->allocated_projects_count ?? 0) }}</p>
                            </div>
                        </div>
                    </article>
                @endforeach
