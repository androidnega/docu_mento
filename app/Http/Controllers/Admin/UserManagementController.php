...
class UserManagementController extends Controller
{
    ...
    public function update(Request $request, User $user): RedirectResponse
    {
        ...
        $canEditPhone = Schema::hasColumn('users', 'phone')
            && ($isSuperAdmin || ($currentUser && $currentUser->id === $user->id));
        if ($canEditPhone && $request->has('phone')) {
            $rules['phone'] = 'nullable|string|max:20';
            if ($request->filled('phone')) {
                $rules['phone_normalized'] = [Rule::unique('users', 'phone')->ignore($user->id)];
            }
            if ($request->filled('phone')) {
                $request->merge(['phone_normalized' => self::normalizePhone($request->phone)]);
            }
        ...
        if ($canEditPhone && $request->has('phone')) {
            $user->phone = $request->filled('phone') ? self::normalizePhone($request->phone) : null;
        }
